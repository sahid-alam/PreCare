...
export default function PatientPage() {
  ...
  const isWorkspaceVisible = status !== "idle" || transcript.length > 0 || symptoms.length > 0 || classification;
  const statusText =
    status === "active"
      ? "In session"
      : status === "connecting"
  ...
  return (
    <div className="min-h-screen bg-[#f4f1e9] text-[#14241c]">
      ...
      <div className="pc-instrument no-print">
        ...
        <span>Asha v1</span>
        <span>{lang === "en" ? "English" : "Hindi"}</span>
        <Link href="/admin" className="ml-auto text-[#bfc8c2] transition-colors hover:text-white">
          Admin dashboard
        </Link>
      </div>
